...
# CREATE SERVER
openssl req -newkey rsa:4096 -nodes -keyout /etc/mysql/ssl/server-key.pem -out /etc/mysql/ssl/server-req.pem -subj "/C=SK/ST=BB/L=Bratislava/O=STU/CN=DB SERVER"
...
# CREATE CLIENT
openssl req -newkey rsa:4096 -nodes -keyout /etc/mysql/ssl/client-key.pem -out /etc/mysql/ssl/client-req.pem -subj "/C=SK/ST=BB/L=Bratislava/O=STU/CN=DB CLIENT"
